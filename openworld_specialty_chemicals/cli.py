...
app = typer.Typer(
    ...
    help="OpenWorld Specialty Chemicals CLI - Environmental compliance monitoring and reporting"
)
...
def _validate_csv_format(df: pd.DataFrame, required_columns: list[str], file_path: str) -> None:
    ...
    if missing:
        console.print(
            f"[red][ERROR] CSV file {file_path} is missing required columns: "
            f"{', '.join(sorted(missing))}[/red]"
        )
...
def main(
    ...
    log_format: str = typer.Option(
        "text", "--log-format", help="Log format: text or json", metavar="{text|json}"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Do not modify files or make network calls; print planned actions"
    ),
    ...
    if verbose and not quiet:
        ...
        log.setLevel("DEBUG")
        console.print(
            f"[blue][CONFIG] Verbose logging enabled | corr_id={corr_id} "
            f"| format={log_format}[/blue]"
        )
    else:
        ...
        if quiet:
            log.setLevel("ERROR")
        console.print(
            f"[blue][CONFIG] corr_id={corr_id} | format={log_format} "
            f"| quiet={quiet} | dry_run={dry_run}[/blue]"
        )
...
def process_chemistry(
    input: str = typer.Option(
        ..., "--input", "-i", help="Input CSV file with time-series concentration data"
    ),
    species: str = typer.Option(
        ..., "--species", "-s", help="Chemical species code (e.g., SO4, As, Ni)"
    ),
    ...
    try:
        ...
        if not QUIET:
            console.print(
                f"[blue][CHEMISTRY] Processing chemistry data for species: {species}[/blue]"
            )
        ...
        if species_col in df.columns:
            ...
            if len(df.columns) < 2:
                console.print(
                    "[red][ERROR] Error: Wide format CSV must have at least time "
                    "and concentration columns[/red]"
                )
            ...
            if not QUIET:
                console.print(
                    f"[yellow][CONVERT] Converted wide format to tidy "
                    f"(time column: {time_col})[/yellow]"
                )
        else:
            # Assume tidy format
...
def monitor_batch(
    input: str = typer.Option(
        ..., "--input", "-i", help="Input CSV with monitoring data"
    ),
    permit: str = typer.Option(
        "", "--permit", "-p", help="Permit JSON file (uses default limits if not specified)"
    ),
    out: str = typer.Option(
        "artifacts/alerts.json",
        "--out",
        "-o",
        help="Output JSON file for compliance alerts"
    ),
):
    """
    ...
    try:
        ...
        if not QUIET:
            console.print("[blue][MONITOR] Monitoring batch data for compliance...[/blue]")
        ...
        if not QUIET:
            console.print(
                f"[blue][INFO] Using {'custom permit' if permit else 'default permit limits'}[/blue]"
            )
...
def simulate_stream_cmd(
    source: Path = typer.Option(
        ..., "--source", "-s", help="Source CSV with time,species,concentration columns"
    ),
    delay: float = typer.Option(0.1, "--delay", "-d", help="Delay between records in seconds"),
    publish_ws: Optional[str] = typer.Option(
        None, "--publish-ws", help="WebSocket URL to publish stream data"
    ),
    out: Optional[Path] = typer.Option(
        None,
        "--out",
        "-o",
        help="JSONL output file (prints to stdout if not specified)"
    ),
):
    """
        ...
        if publish_ws and not DRY_RUN:
            ...
            async def _publish_stream() -> None:
                try:
                    ...
                    console.print(
                        f"[green][SUCCESS] Published {len(rows)} records to WebSocket[/green]"
                    )
...
def cert(
    alerts: str = typer.Option(
        ...
        "--alerts",
        "-a",
        help="Alerts JSON file from monitor-batch command"
    ),
    ...
    out: str = typer.Option(
        ...
        "--out",
        "-o",
        help="Output HTML certificate file"
    ),
    ...
    try:
        ...
        if isinstance(payload, dict) and "alerts" in payload:
            ...
            console.print(
                "[red][ERROR] Error: Invalid alerts file format - expected array "
                "or object with 'alerts' key[/red]"
            )
...
def advise(
    ...
    try:
        ...
        console.print(
            f"[blue][ADVICE] Generating remediation advice"
            f"{'for site: ' + site if site else ''}[/blue]"
        )
        ...
        console.print(
            f"[blue][INFO] Analyzing {len(alert_list)} alerts "
            f"for remediation strategies[/blue]"
        )
        ...
        if output_format == "json":
            ...
            if actions:
                table = Table(
                    title=f"[ADVICE] Remediation Recommendations"
                    f"{' - ' + site if site else ''}"
                )
...
def dashboard(
    ...
    serve: bool = typer.Option(
        True,
        "--serve/--no-serve",
        help="Start the dashboard server (use --no-serve to just validate config)"
    ),
    ...
    try:
        if not QUIET:
            console.print("[blue][WEB] Dashboard configuration[/blue]")
        ...
        console.print(
            f"[blue][START] Starting dashboard server"
            f"{'with auto-reload' if reload else ''}...[/blue]"
        )
        ...
        try:
            ...
            console.print(
                "[red][ERROR] Error: uvicorn not available. Install with: "
                "pip install uvicorn[/red]"
            )
            _handle_error("dashboard dependency", e)
        except OSError as e:
            if "Address already in use" in str(e):
                console.print(
                    f"[red][ERROR] Error: Port {port} is already in use. "
                    "Try a different port with --port[/red]"
                )
            else:
