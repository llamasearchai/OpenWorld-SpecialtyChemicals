...
def evaluate_permit(df, permit: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Handle both column format styles: species columns (SO4_mgL) and tidy format (species, concentration)
        ...
        for sp, limit in limits.items():
            ...
            if current_val > float(limit):
                ...
                alerts.append({
                    ...
                    "message": f"{sp} concentration {current_val:.3f} mg/L exceeds limit {float(limit):.3f} mg/L"
                })
            ...
            if len(concentrations) >= window:
                ...
                if trend_val > float(limit):
                    ...
                    alerts.append({
                        ...
                        "limit": float(limit), 
                        "action": actions.get(sp, f"Review {sp} treatment process"),
                        "message": f"{sp} rolling mean {trend_val:.3f} mg/L over {window} readings exceeds limit {float(limit):.3f} mg/L"
                    })
    else:
        ...
        for sp, limit in limits.items():
            ...
            if x[-1] > float(limit):
                ...
                alerts.append({
                    ...
                    "limit": float(limit), 
                    "action": actions.get(sp, f"Review {sp} treatment process"),
                    "message": f"{sp} concentration {x[-1]:.3f} mg/L exceeds limit {float(limit):.3f} mg/L"
                })
            ...
            if len(x) >= window:
                ...
                if rm[-1] > float(limit):
                    ...
                    alerts.append({
                        ...
                        "limit": float(limit), 
                        "action": actions.get(sp, f"Review {sp} treatment process"),
                        "message": f"{sp} rolling mean {rm[-1]:.3f} mg/L over {window} readings exceeds limit {float(limit):.3f} mg/L"
                    })
